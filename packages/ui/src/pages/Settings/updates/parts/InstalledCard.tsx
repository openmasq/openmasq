      ...
      <div className="ver-now om-sweep-host">
        ...
        <div className="flex-min">
          <div className="ver-now-name">
            <span className="om-sweep">{BRAND.name} {current?.version ?? "—"}</span>
          </div>
          <div className="ver-now-chan">
            Canal <span className="font-mono">{current?.channel ?? "—"}</span>
            {(!status || status.state === "not-available") && " · à jour"}
          </div>
          {current?.installId && (
            <button
              onClick={copyInstallId}
              title="Copier l'ID de cet appareil (pour accorder l'accès côté opérateur)"
              className="ver-now-id"
            >
              {copiedId ? (
                <>
                  <CheckIcon size={11} /> ID copié
                </>
              ) : (
        ...
        {downloaded ? (
          ...
          <button onClick={check} className="ver-btn">
            <RefreshIcon size={14} /> <span className="om-sweep">Rechercher les mises à jour</span>
          </button>
        )}
      </div>
        ...
        <div className="ver-status">
          ...
          {status?.state === "downloading" && (
            <div className="ver-progress">
              ...
              <div className="ver-progress-fill" style={{ width: `${Math.round(status.percent ?? 0)}%` }} />
            </div>
          )}
        </div>
      )}
