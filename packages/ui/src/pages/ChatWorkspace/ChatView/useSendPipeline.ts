...
export function useSendPipeline(p: ChatViewProps, d: Deps) {
  ...
  function submit() {
    ...
    // Pre-conversation manual redactions ride the send; once a conversation exists they live on it.
    const forcedRedactions = conversation || !d.forced.pendingForced.length ? undefined : d.forced.pendingForced;
    d.clearInput();
    d.intents.resetAll();
    d.att.setAttachments([]);
    if (usable.length > 0) {
      void runSend(text, usable, { competence, askTarget, docReplacements: reuseDocReplacements(usable) });
      return;
    }
    d.forced.clearPendingForced();
    void runSend(text, usable, { plotTag: activeTag?.tag, competence, askTarget, forcedRedactions });
  }
