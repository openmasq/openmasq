...
export function MemoryNodePanel({
  ...
  return (
    <aside className="om-mem-panel om-step-in" aria-label="Détail du nœud">
      ...
      {card ? (
        <div className="om-mem-panel-body">
          ...
          <div className="om-skill-filters" role="radiogroup" aria-label="Catégorie">
          ...
          <textarea
            ...
            placeholder="Les faits durables (préférences, contexte, décisions)."
            aria-label="Faits"
          />
          ...
          {(card.factsLog?.length ?? 0) > 0 && (
            <div className="om-mem-history">
              <div className="cv-eyebrow">Remplacé par une mise à jour</div>
                ...
                <div key={`${e.at}-${i}`} className="om-mem-history-row">
                  ...
                  <span className="om-mem-history-meta">
                    ...
                    <button
                      ...
                      title="Remettre cette phrase dans la fiche — la version actuelle passe dans l'historique"
                      onClick={() => {
      ...
      <div className="om-mem-panel-links">
        ...
        {linked.length === 0 && <p className="om-skill-desc">Aucune connexion.</p>}
      </div>
