...
import { SkillSuggestPane } from "./parts/SkillSuggestPane";
...
const catOptions = (t: Messages): HueOption[] =>
  skillCategories(t).map((c) => ({ value: c.id, label: c.label, tone: c.tone, glyph: c.glyph }));
...
export function skillToDraft(c: Skill): SkillDraft {
  return {
    id: c.id,
    name: c.name,
    desc: c.desc ?? "",
...
export function SkillModal({
  ...
  const patch = (p: Partial<SkillDraft>) => setDraft((d) => ({ ...d, ...p }));
  ...
  const cat = skillCategory(draft.cat, t);
