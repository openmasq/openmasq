...
export function makeVaultTerm(value: string, token: string, note?: string): VaultTerm {
  const id =
...
export function combinedVaultTerms(
  s: { coffre?: VaultTerm[]; orgCoffre?: VaultTerm[] } | undefined,
): VaultTerm[] {
...
export function vaultTermsToForced(vaultTerms: VaultTerm[] | undefined): { value: string; category: string }[] {
  return (vaultTerms ?? []).filter((t) => t.value.trim()).map((t) => ({ value: t.value, category: t.token }));
}

/** True when a value already lives in the Coffre (case-insensitive, trimmed) — so the
 *  UI can avoid duplicate entries. */
export function vaultHasValue(vaultTerms: VaultTerm[] | undefined, value: string): boolean {
  const v = value.trim().toLowerCase();
  return !!v && (vaultTerms ?? []).some((t) => t.value.trim().toLowerCase() === v);
}

export interface VaultTermUse {
  convId: string;
...
export interface VaultTermOccurrences {
  uses: VaultTermUse[];
  totalCount: number;
  convCount: number;
}
 ...
 */
export function vaultTermOccurrences(term: VaultTerm, conversations: Conversation[]): VaultTermOccurrences {
  const value = term.value.trim();
  const valueLc = value.toLowerCase();
  const uses: VaultTermUse[] = [];
