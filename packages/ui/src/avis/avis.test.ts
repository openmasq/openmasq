...
describe("canSendFeedback", () => {
  ...
  it("drops the mood requirement once the journal actually rides", () => {
    const withJournal = draft({ mood: null, journal: "wire…", attachJournal: true });
    expect(canSendFeedback(withJournal)).toBe(true);
    // …but a message of substance is still required — the journal is context, not a report.
    expect(canSendFeedback({ ...withJournal, message: "ok" })).toBe(false);
  });
  ...
  it("does NOT drop it when the journal is present but switched OFF", () => {
    ...
    expect(canSendFeedback(draft({ mood: null, journal: "wire…", attachJournal: false }))).toBe(false);
    expect(canSendFeedback(draft({ mood: null, journal: "   ", attachJournal: true }))).toBe(false);
  });
});
...
describe("buildFeedback", () => {
  ...
  it("OMITS the mood key entirely on a journal report, never sends a null", () => {
    ...
    const f = buildFeedback(draft({ mood: null, journal: "wire…", attachJournal: true }), ctx);
  ...
  it("attache un journal présent par défaut ; seul un refus explicite le retire", () => {
    ...
    expect(buildFeedback(draft({ attachJournal: true }), ctx)).not.toHaveProperty("journal");
  });

  it("re-caps an over-cap journal at the choke point, keeping the TAIL (recent entries)", () => {
    const long = "x".repeat(MAX_AVIS_JOURNAL) + "FIN";
    const f = buildFeedback(draft({ journal: long, attachJournal: true }), ctx);
    expect(f?.journal).toHaveLength(MAX_AVIS_JOURNAL);
