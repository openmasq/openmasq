...
import { logExportFor } from "./DebugLogModal/entryText";
import {
  buildFeedback,
  canSendFeedback,
  capLog,
...
import { feedbackCategories, feedbackMoods } from "../../feedback/vocabulary";
...
export function FeedbackModal({
  ...
  const moodOptional = carriesLog(draft);
   ...
   */
  const offer = useMemo(
    () => (draft.category === "bug" && !draft.journal ? capLog(logExportFor(convId)) : ""),
    [draft.category, draft.journal, convId],
  );
  const log = draft.journal || offer;
  ...
  const submit = async () => {
    ...
    const payload = buildFeedback({ ...draft, journal: log }, context);
    if (!payload || !host.feedback) return;
    setBusy(true);
    setError(null);
    try {
      await host.feedback.send(payload);
          ...
          <div className="om-avis-body">
            ...
            {log && (
              <div className="om-avis-journal">
                ...
                {draft.attachLog !== false && (
                  <pre className="om-avis-journal-preview">{log}</pre>
                )}
              </div>
            )}
