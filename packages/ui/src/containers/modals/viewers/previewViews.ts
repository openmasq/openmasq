...
export function previewViews(s: PreviewShape, file: PreviewFile): DocViewOption[] {
  const views: DocViewOption[] = [];
  if (s.isImage && s.hasBytes) views.push({ id: "image", label: "Image" });
  if (s.isPdf && s.hasBytes)
    views.push({ id: "pdf", label: "Pages redacted", shield: true, hint: "Les pages, fausses valeurs peintes dessus" });
  if (s.isRich && s.hasBytes)
    views.push({
      id: "rich",
      label: s.isSheet ? "Feuille" : s.isPptx ? "Présentation" : "Document",
      ...
      ...(s.isSheet ? { hint: "Le fichier tel quel, avant redaction" } : {}),
    });
  if (s.isMd && !!file.text) views.push({ id: "rendu", label: "Rendu" });
  ...
  if (file.text && !s.isRich)
    views.push({ id: "original", label: "Original", hint: "Le fichier tel quel, avant redaction" });
  if (file.text)
    // « Ce qui », not « le texte qui » — for a spreadsheet this layer is a GRID.
    views.push({ id: "redacted", label: "Redacted", shield: true, hint: "Ce qui quittera la machine" });
  if (s.hasOcrLayer) views.push({ id: "ocr", label: "Texte de l'image", hint: "Ce que disent les pixels de la page" });
  return views;
}
