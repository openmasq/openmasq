...
const { logExportFor } = vi.hoisted(() => ({ logExportFor: vi.fn() }));
vi.mock("./DebugLogModal/entryText", () => ({ logExportFor }));

import { mount } from "../../testKit";
import { FeedbackModal } from "./FeedbackModal";
import type { Feedback } from "../../feedback/feedback";

const LOG = "[10:02:03] WIRE → gpt-5\nBonjour [PERSON1]";
...
describe("AvisModal — the debug journal offer on a Bug report", () => {
  ...
  it("offers the journal as soon as the category is Bug", async () => {
    ...
    const { m, pickCategory } = await openFeedback();
  ...
  it("arrive PRÉ-COCHÉ, l'aperçu verbatim à l'écran, et le journal part avec l'envoi", async () => {
    ...
    const { m, pickCategory, sendIt } = await openFeedback();
    await pickCategory("Bug");
    expect(m.find(".om-avis-journal [role=switch]").getAttribute("aria-checked")).toBe("true");
    expect(m.find(".om-avis-journal-preview").textContent).toBe(LOG);
    expect((await sendIt()).journal).toBe(LOG);
  });
