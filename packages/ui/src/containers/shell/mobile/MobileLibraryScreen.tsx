  ...
  return (
    <div className="mobile-screen mlib">
      <header className="mlib-head">
        <h1 className="mlib-title">Bibliothèque</h1>
        <div className="mobile-seg" role="tablist" aria-label="Fichiers ou images">
          {(
            [
              ["files", "Fichiers"],
              ["images", "Images"],
      ...
      <div className="mlib-body">
        {files === null ? (
          <p className="mlib-empty">Chargement…</p>
        ) : visible.length === 0 ? (
          <p className="mlib-empty">
            {segment === "images" ? "Aucune image." : "Aucun fichier."}
            <span className="mlib-empty-sub">
              Les pièces jointes de vos conversations atterrissent ici, déjà redacted.
            </span>
          </p>
        ) : segment === "images" ? (
      ...
      <AnimatePresence>
        {confirmDelete && (
          <ConfirmDialog
            ...
            message={`« ${confirmDelete.name} » sera définitivement supprimé de la bibliothèque (fichier original + version redacted). Cette action est irréversible.`}
...
function FileRow({
  ...
  return (
    <li className="mlib-row">
      ...
      <button type="button" className="mlib-row-menu" onClick={onMenu} aria-label={`Actions — ${file.name}`}>
        <DotsIcon size={18} />
      </button>
    </li>
  );
