...
export function VaultSection({
  ...
  const { chat, go } = shell;
  const host = useHost();
  const [promo, setPromo] = useState<CoffreTerm | null>(null);
  return (
    <>
      <VaultView
        ...
        loaded={chat.loaded}
        onAdd={chat.addCoffreTerm}
        onRemove={chat.removeCoffreTerm}
          ...
          <SharePromoteModal
            ...
            onShare={async (audience) =>
              !!(await host.orgShares?.proposeCoffre({
                audience,
                label: `Terme (${coffreTypeLabel(promo.token)})${promo.note ? ` — ${promo.note}` : ""}`,
                terms: [promo],
              }))
            }
          />
