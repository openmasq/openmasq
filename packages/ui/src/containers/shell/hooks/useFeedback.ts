...
import type { FeedbackOpenApi } from "../../providers/feedbackOpen";
import type { FeedbackContext, FeedbackDraft } from "../../../feedback/feedback";
 ...
 */
export function useFeedback({ chat, section }: { chat: ChatStore; section: Section }): {
  ...
  const api = useMemo<FeedbackOpenApi>(
    () => (host.feedback ? { openFeedback: (prefill) => setOpen({ prefill }) } : {}),
    [host.feedback],
  );
