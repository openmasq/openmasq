 ...
 */
const OpenSkillCtx = createContext<((id: string) => void) | null>(null);

export const SkillOpenProvider = OpenSkillCtx.Provider;

/** `null` when no provider is mounted — callers hide the affordance rather than
 *  render a link that does nothing. */
export function useOpenSkill(): ((id: string) => void) | null {
  return useContext(OpenSkillCtx);
}
