...
import { mergeLegacyWorkflows, workflowToSkill } from "./migrate";
...
const wf = (id: string, over: Partial<Skill> = {}): Skill =>
  ({
    id,
    name: `Routine ${id}`,
    prompt: "corps",
...
const comp = (id: string): Skill =>
  ({ id, name: `Compétence ${id}`, prompt: "corps", cat: "redaction", createdAt: 1 }) as Skill;

describe("workflowToCompetence", () => {
  it("range un ex-workflow en « Routines » et lui garde ses connecteurs", () => {
    const c = workflowToSkill(wf("a"));
  ...
  it("une liste de connecteurs VIDE disparaît — le champ doit rester le test de « pilote des outils »", () => {
    expect(workflowToSkill(wf("a", { servers: [] })).servers).toBeUndefined();
  });

  it("une catégorie déjà posée n'est pas réécrite", () => {
    expect(workflowToSkill(wf("a", { cat: "code" })).cat).toBe("code");
  });
});
