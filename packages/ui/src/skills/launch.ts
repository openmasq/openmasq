...
export interface SkillServerMeta {
  id: string;
  name: string;
...
export function skillServerMeta(id: string): SkillServerMeta {
  const c = findConnector(id);
  return { id, name: c?.name ?? id, tone: c?.tone ?? "violet" };
}

export function skillServers(c: Pick<Skill, "servers">): SkillServerMeta[] {
  return (c.servers ?? []).map(skillServerMeta);
}

/** True when the compétence drives tools — the only test that decides a
 *  different behaviour, everywhere. */
export function drivesTools(c: Pick<Skill, "servers">): boolean {
 ...
 */
export function skillLaunchText(c: Pick<Skill, "prompt" | "servers">): string {
  const names = skillServers(c).map((s) => s.name);
 ...
 */
export function activeSkillScope(
  messages: readonly {
    role: string;
