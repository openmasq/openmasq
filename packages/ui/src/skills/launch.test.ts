...
import { activeSkillScope, cappedSlots, skillLaunchText, drivesTools, promptSlots } from "./launch";
...
describe("competenceLaunchText", () => {
  it("sans connecteur ni accolade, le prompt part NU — le comportement d'avant la fusion", () => {
    expect(skillLaunchText({ prompt: "Résume ce texte.", servers: [] })).toBe(
      "Résume ce texte.",
    );
    expect(skillLaunchText({ prompt: "Résume ce texte." })).toBe("Résume ce texte.");
  });

  it("avec des connecteurs, une ligne de consigne les NOMME", () => {
    const out = skillLaunchText({ prompt: "Fais le point.", servers: ["gmail"] });
  ...
  it("les accolades sont expliquées AU MODÈLE, et il lui est interdit d'en inventer une", () => {
    const out = skillLaunchText({ prompt: "Prépare ma journée du {date}.", servers: [] });
...
describe("activeCompetenceScope", () => {
  ...
  it("survit à un tour nu", () => {
    expect(
      activeSkillScope([user({ competence: { servers: ["gmail"] } }), user()]),
    ).toEqual(["gmail"]);
  });
  ...
  it("lit l'ANCIEN tag `workflow` aussi bien que le neuf", () => {
    expect(activeSkillScope([user({ workflow: { servers: ["gmail"] } })])).toEqual(["gmail"]);
  });
  ...
  it("aucune compétence à outils ⇒ aucune portée (les outils du tour restent ceux d'habitude)", () => {
    expect(activeSkillScope([user(), user({ competence: {} })])).toBeUndefined();
  });

  it("un tour de l'ASSISTANT ne définit jamais de portée", () => {
    expect(
      activeSkillScope([{ role: "assistant", competence: { servers: ["gmail"] } }]),
    ).toBeUndefined();
  });
});
