...
// Brands the INSTALLED `simple-icons` no longer ships (removed upstream on brand-
// guideline caution): their 24×24 paths + official hexes are vendored VERBATIM from
// the last official npm release that carried them (simple-icons@10.4.0, CC0) — same
// license, same accuracy, no third-party mirror. Trademarks belong to their owners;
// the glyphs identify the connector, nothing more (nominative use, like the si* set).
...
// Preset id → the real brand logo, from the open-source `simple-icons` set
// (accurate official glyphs; trademarks belong to their owners — shown only to
// identify the connector). Brands `simple-icons` never carried (Exa, Tavily,
// Firecrawl, Apify, Amplitude, Bright Data, Jotform, WebsitePublisher,
// Morningstar, Vantage, Synapse, Superhuman, Fireflies, monday, Attio, Close,
// Semgrep) get a raster favicon instead, in ./mcpLogoImages.ts. Only the local
// `filesystem` server, the built-in browser, the `demo` broker and any
// user-added custom server fall back to the coloured letter tile.
export const MCP_LOGOS: Record<string, BrandLogo> = {
  ...
  dropbox: { path: siDropbox.path, hex: `#${siDropbox.hex}` },
  "google-drive": { path: siGoogledrive.path, hex: `#${siGoogledrive.hex}` },
  "google-calendar": { path: siGooglecalendar.path, hex: `#${siGooglecalendar.hex}` },
  "google-docs": { path: siGoogledocs.path, hex: `#${siGoogledocs.hex}` },
  "google-sheets": { path: siGooglesheets.path, hex: `#${siGooglesheets.hex}` },
  "google-tasks": { path: siGoogletasks.path, hex: `#${siGoogletasks.hex}` },
  "google-analytics": { path: siGoogleanalytics.path, hex: `#${siGoogleanalytics.hex}` },
  ...
  square: { path: siSquare.path, hex: `#${siSquare.hex}` },
  zapier: { path: siZapier.path, hex: `#${siZapier.hex}` },
  wix: { path: siWix.path, hex: `#${siWix.hex}` },
  ...
  posthog: { path: siPosthog.path, hex: `#${siPosthog.hex}` },
  ...
  "microsoft-onedrive": MS_ONEDRIVE,
  "microsoft-sharepoint": MS_SHAREPOINT,
