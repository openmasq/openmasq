...
export function competenceSuggestions(t: Messages): CompetenceSuggestion[] {
  return COMPETENCE_SHAPE.map((c) => ({ ...c, ...t.templates.competences[c.id] }));
}
...
export function suggestedCompetences(
  ...
  return pickSuggestions(competenceSuggestions(t), existing, limit);
}
