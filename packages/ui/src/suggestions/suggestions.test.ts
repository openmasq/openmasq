...
describe("COMPETENCE_SUGGESTIONS", () => {
  it("are all saveable as-is (the create bar `makeCompetence` enforces)", () => {
    for (const s of COMPETENCE_SUGGESTIONS) expect(asCompetence(s)).not.toBeNull();
  });

  it("carry a known category and unique ids/names", () => {
    const cats = new Set(COMPETENCE_CATEGORIES.map((c) => c.id));
    for (const s of COMPETENCE_SUGGESTIONS) expect(cats.has(s.cat)).toBe(true);
    expect(new Set(COMPETENCE_SUGGESTIONS.map((s) => s.id)).size).toBe(
      COMPETENCE_SUGGESTIONS.length,
    );
    expect(new Set(COMPETENCE_SUGGESTIONS.map((s) => s.name)).size).toBe(
      COMPETENCE_SUGGESTIONS.length,
    );
  });
   ...
   * de la rédaction.
   *
   * « Routines » est servie par l'AUTRE catalogue (`ROUTINE_SUGGESTIONS`) : elle a une
  ...
  it("cover every category IN THE OFFERED SET — the strip is capped, so catalog order is what a user actually sees", () => {
    const shown = new Set(suggestedCompetences([]).map((s) => s.cat));
  ...
  it("« Routines » est servie par l'autre catalogue — la fusion n'a laissé aucune catégorie vide", () => {
    const offered = offeredTemplates([], { limit: 99 });
  ...
  it("stop being offered once saved — a picked template never re-appears", () => {
    const saved = [asCompetence(COMPETENCE_SUGGESTIONS[0])];
    const offered = suggestedCompetences(saved, 99);
    expect(offered.some((s) => s.id === COMPETENCE_SUGGESTIONS[0].id)).toBe(false);
    expect(offered).toHaveLength(COMPETENCE_SUGGESTIONS.length - 1);
  });
});
...
describe("ROUTINE_SUGGESTIONS", () => {
  ...
  it("leads with a template that needs NO ACCOUNT — a first-run user has nothing connected, so catalog order is all they get", () => {
    const first = suggestedRoutines([])[0];
  ...
  it("keeps the dev-only routines OUT of the capped default strip", () => {
    const shown = suggestedRoutines([]).flatMap((s) => s.servers);
    for (const devOnly of ["github", "linear", "sentry"]) expect(shown).not.toContain(devOnly);
    expect(suggestedRoutines([]).map((s) => s.id)).toContain("revue-boite-mail");
  });
  ...
  it("has unique ids and names, and every template names at least one connector", () => {
    expect(new Set(ROUTINE_SUGGESTIONS.map((s) => s.id)).size).toBe(ROUTINE_SUGGESTIONS.length);
    expect(new Set(ROUTINE_SUGGESTIONS.map((s) => s.name)).size).toBe(ROUTINE_SUGGESTIONS.length);
    for (const s of ROUTINE_SUGGESTIONS) expect(s.servers.length).toBeGreaterThan(0);
  });

  it("ranks the launchable ones first — a connected connector wins the top slot", () => {
    const last = ROUTINE_SUGGESTIONS[ROUTINE_SUGGESTIONS.length - 1];
    const offered = suggestedRoutines([], { connected: new Set(last.servers) });
    expect(offered[0].id).toBe(last.id);
  });

  it("falls back to catalog order with nothing connected", () => {
    expect(suggestedRoutines([], { connected: new Set(), limit: 99 }).map((s) => s.id)).toEqual(
      ROUTINE_SUGGESTIONS.map((s) => s.id),
    );
  });

  it("drops a template naming a connector this host cannot offer (a dead routine, not a hint)", () => {
    const offered = suggestedRoutines([], { unavailable: new Set(["browser"]), limit: 99 });
    ...
    expect(offered).toHaveLength(
      ROUTINE_SUGGESTIONS.filter((s) => !s.servers.includes("browser")).length,
    );
  });
  ...
  it("always keeps ONE routine needing a new connection — the strip is how a 2nd integration is discovered", () => {
    ...
    const offered = suggestedRoutines([], { connected });
  ...
  it("drops one already saved", () => {
    const saved = [
      makeCompetence({ name: ROUTINE_SUGGESTIONS[0].name, prompt: "x" }) as Competence,
    ];
    expect(suggestedRoutines(saved, { limit: 99 }).map((s) => s.id)).not.toContain(
      ROUTINE_SUGGESTIONS[0].id,
    );
  });
});

describe("focusRoutines — ticking an integration always answers about IT", () => {
  const ranked = suggestedRoutines([], { limit: 99 });
  ...
  it("REGRESSION: an integration no curated template names still gets its own idea", () => {
    ...
    const items = focusRoutines(ranked, new Set(["microsoft-outlook"]));
  ...
  it("EVERY connector in the catalog answers with an idea scoped to itself", () => {
    ...
    for (const c of MCP_CONNECTORS) {
      const items = focusRoutines(ranked, new Set([c.id]));
  ...
  it("prefers the CURATED idea when one exists, and doesn't add a generic twin", () => {
    const items = focusRoutines(ranked, new Set(["gmail"]));
  ...
  it("mixes curated + generated when the ticks span both, one per unserved service", () => {
    const items = focusRoutines(ranked, new Set(["gmail", "microsoft-outlook"]));
  ...
  it("un-ticking everything goes back to the general list", () => {
    expect(focusRoutines(ranked, new Set()).map((s) => s.id)).toEqual(
...
describe("genericRoutineFor", () => {
  ...
  it("returns nothing for an unknown id rather than inventing a service", () => {
    expect(genericRoutineFor("defunct-connector")).toBeUndefined();
  });
});

describe("ownKeysNeeded — « il faut vos propres clés pour ça »", () => {
  const byId = (id: string) => ROUTINE_SUGGESTIONS.find((s) => s.id === id)!;
