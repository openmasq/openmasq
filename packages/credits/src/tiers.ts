...
// This is the SINGLE source-of-truth for the ALLOTMENT (Solo 800 / Team 800): the billing
// side DERIVES its amounts from here via `creditsCentsForAccountType`, so there is no
// hand-kept duplicate to drift (rule 9). FREE/PRO have no budget → platform models are
// subscription-only.
//
...
//
// ⚠️ SCALE is RETIRED from the sellable catalog but KEEPS its allotment on purpose: a
// subscription taken before the retirement keeps billing, and dropping the row would
// silently grant those seats 0 credit. Same treatment as the legacy PRO row: unreachable
// for a NEW purchase, still honoured for an old one.
