 ...
 *
 *  - {@link emitCoffreRecords} — one `coffre` record per new/changed term, one
 *    `coffreTombstone` per locally-deleted one. Unchanged set → nothing.
 *  - {@link absorbCoffreRecords} — fold pulled records into the local list.
...
export interface SyncedCoffreTerm {
  id: string;
  value: string;
  token: string;
  note?: string;
...
export interface CoffreSyncState {
  accountId: string;
  lamport: number;
...
export const emptyCoffreSyncState = (accountId: string): CoffreSyncState => ({
  accountId,
  lamport: 0,
  sigs: {},
});
...
const cleanTerm = (t: SyncedCoffreTerm): SyncedCoffreTerm => ({
  id: t.id,
  value: t.value,
  token: t.token,
...
type CoffrePayload = { type: "coffreTerm"; item: SyncedCoffreTerm };
...
const sigOf = (p: CoffrePayload): string => JSON.stringify(p);

function entriesOf(terms: SyncedCoffreTerm[]): Map<string, CoffrePayload> {
  const out = new Map<string, CoffrePayload>();
...
function validPayload(raw: unknown): CoffrePayload | null {
  const p = raw as CoffrePayload | null;
  ...
  if (!item || !str(item.id) || !str(item.value) || !str(item.token) || !num(item.createdAt))
    return null;
  return { type: "coffreTerm", item: cleanTerm(item as unknown as SyncedCoffreTerm) };
}
...
export function emitCoffreRecords(
  current: SyncedCoffreTerm[],
  state: CoffreSyncState,
  deviceId: string,
): { records: SyncRecord[]; state: CoffreSyncState } {
...
export function absorbCoffreRecords(
  local: SyncedCoffreTerm[],
  pulled: SyncRecord[],
  state: CoffreSyncState,
): { terms: SyncedCoffreTerm[]; state: CoffreSyncState; changed: boolean } {
  const merged = mergeRecords([], pulled);
  const view = liveView(merged);
  const remote = new Map<string, CoffrePayload>();
  ...
  const applyRemote = (key: string, p: CoffrePayload) => {
