#!/usr/bin/env node
// i18n ratchet — "fully translated" is an invariant held by the tool, not by vigilance
// (same shape as `check-file-size.mjs`). It freezes today's count of hard-coded copy per
// file: a NEW file carrying copy fails, a frozen file that GROWS past its count fails, and
// migrating a string to `@openmasq/i18n` tightens the ratchet (`--update` re-freezes
// downward; never upward without `--allow-growth`).
//
// What it counts is a PROXY for hard-coded copy, not a proof: a string literal on a
// property someone READS (`title`, `aria-label`, `placeholder`, `label`…), a JSX text node,
// both branches of a string ternary, and a TEMPLATE literal whose text reads as French.
// ⚠️ Accent-blind on purpose: hard-coded English is the same defect as hard-coded French
// in an app that ships two languages.
//
...
/** The JSX form (`title="…"`) AND the TABLE form (`desc: "…"`): both carry copy.
 *  `name` is OUT: a connector or model name is a proper noun, nothing to translate. */
...
const JSX_TEXT = />([^<>{}\n]{3,}?)</g;
/** A TEMPLATE literal (`Mise à jour ${v}`) is a sentence with a hole. Counted when the text
 *  around the holes reads as French: an accented letter, or a function word no identifier,
 *  class list or URL carries. */
const TEMPLATE = /`([^`]{4,}?)`/g;
const FRENCH_WORDS =
