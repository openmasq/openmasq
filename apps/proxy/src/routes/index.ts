// Mount order IS the allow-list: the dead end first (`/blocked`, refused whatever the method),
// then the masked commands (at `/` and under their explicit family prefix — Mistral under its
// prefix ONLY), the passthrough last — it relays a text-less method and refuses the rest.
import { Router } from "express";
import type { RelayDeps } from "../lib/relay.js";
import mountAnthropicRoutes from "../features/anthropic/routes.js";
import mountBlocked, { BLOCKED_PREFIX } from "../features/blocked/routes.js";
import mountGeminiRoutes from "../features/gemini/routes.js";
import mountMistralRoutes from "../features/mistral/routes.js";
...
export function apiRouter(deps: RelayDeps): Router {
  ...
  router.use(BLOCKED_PREFIX, mountBlocked(Router(), deps));
  router.use("/openai", openai);
  router.use("/anthropic", anthropic);
  router.use("/gemini", gemini);
  router.use("/mistral", mountMistralRoutes(Router(), deps));
  router.use(openai);
