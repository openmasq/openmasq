...
function toRequest(argv: string[]): { cmd: string; args: Record<string, unknown> } {
  const [cmd, ...rest] = argv;
  const first = rest[0];
  switch (cmd) {
    case "start":
      return { cmd, args: first ? (JSON.parse(first) as Record<string, unknown>) : {} };
    case "look":
      return { cmd, args: { nom: first ?? "vue" } };
    case "click":
      return { cmd, args: { nom: first, n: rest[1] ? Number(rest[1]) : 1 } };
    case "type":
      return { cmd, args: { texte: first, champ: rest[1] } };
    case "key":
      return { cmd, args: { touche: first } };
    case "ask":
      return { cmd, args: { prompt: rest.join(" ") } };
    case "eval":
      return { cmd, args: { js: rest.join(" ") } };
    case "drop":
      return { cmd, args: { chemin: first } };
    default:
...
async function send(req: unknown): Promise<string> {
...
async function main(): Promise<void> {
  ...
  if (argv[0] === "down") {
    if (existsSync(SOCK)) {
      await send({ cmd: "stop" }).catch(() => {});
      if (existsSync(SOCK)) unlinkSync(SOCK);
    }
    process.stdout.write('{"ok":true,"arrete":true}\n');
    return;
  }
  if (!existsSync(SOCK)) await raise();
  if (argv[0] === "up") {
    process.stdout.write('{"ok":true,"pilote":"prêt"}\n');
    return;
  }
  const raw = await send(toRequest(argv));
  ...
  if (!(JSON.parse(raw) as { ok: boolean }).ok) process.exitCode = 1;
}
