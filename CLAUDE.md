...
- **NO worktree, NEVER again** (the old "one session = one worktree" convention is abolished, its machinery dismantled). Everyone works in the SHARED TREE, directly on `dev`; you touch only YOUR files, and a file already modified by another session is not committed with yours. ⚠️ **Finished work is committed and pushed in the turn that finishes it**, never "later" — unpushed work once piled up across six branches, one of which re-applied another's commits. "Finished" means GATES GREEN: `dev` is the trunk every session and every fork PR lands on (the deployments moved to the private `infra` repository on 2026-08-31; here, only a tag builds anything), so work that is blocked or red is SAID OUT LOUD instead of pushed — and is not left to sleep either.
